...
export function DashboardMetrics() {
  ...
  return <div className="grid">{metrics.map(([label, value]) => <div className="panel metric" key={label}><span className="muted">{t(String(label))}</span><strong>{value}</strong></div>)}</div>;
}
