...
export function EntityEditForm({ endpoint, fields, redirectTo, transform }: { endpoint: string; fields: FieldConfig[]; redirectTo: string; transform?: (values: Record<string, string>) => Record<string, unknown>; }) {
  ...
  async function handleCepBlur(field: FieldConfig, value: string) {
    ...
    if (!address) { setCepStatus(t("未找到 CEP / CEP não encontrado")); return; }
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); setError(""); const nextErrors = Object.fromEntries(fields.map((field) => [field.name, validateMaskedValue(field, values[field.name] || "")]).filter(([, message]) => message)); setFieldErrors(nextErrors); if (Object.keys(nextErrors).length) { setSaving(false); return; } const editableValues = Object.fromEntries(fields.map((field) => [field.name, values[field.name] || ""]).filter(([, value]) => value !== "")); const payload = transform ? transform(editableValues) : editableValues; try { await apiFetch(endpoint, { method: "PATCH", body: JSON.stringify(payload) }); router.push(redirectTo); } catch (err) { setError(err instanceof Error ? err.message : t("保存失败 / Falha ao salvar")); } finally { setSaving(false); } }
  return <form className="panel form form-panel" onSubmit={submit}><div className="form-grid">{fields.map((field) => { const options = field.options || dynamicOptions[field.name] || []; return <div className={`field ${field.full ? "full" : ""}`} key={field.name}><label htmlFor={field.name}>{t(field.label)}</label>{field.type === "textarea" ? <textarea id={field.name} value={values[field.name] || ""} required={field.required} onChange={(event) => updateValue(field, event.target.value)} /> : field.type === "select" ? <select id={field.name} value={values[field.name] || ""} required={field.required} onChange={(event) => updateValue(field, event.target.value)}><option value="">{t("请选择 / Selecione")}</option>{options.map((option) => <option key={option.value} value={option.value}>{t(option.label)}</option>)}</select> : <input id={field.name} type={field.type || "text"} inputMode={field.mask ? "numeric" : undefined} value={values[field.name] || ""} required={field.required} onChange={(event) => updateValue(field, event.target.value)} onBlur={(event) => handleCepBlur(field, event.target.value)} />}{fieldErrors[field.name] ? <span className="error">{t(fieldErrors[field.name])}</span> : null}{field.autoFillAddress && cepStatus ? <span className="muted">{cepStatus}</span> : null}</div>; })}</div>{error ? <div className="error">{error}</div> : null}<div className="toolbar-right"><button className="button" type="submit" disabled={saving}>{saving ? t("保存中 / Salvando") : t("保存修改 / Salvar alterações")}</button><button className="button secondary" type="button" onClick={() => router.push(redirectTo)}>{t("取消 / Cancelar")}</button></div></form>;
}
