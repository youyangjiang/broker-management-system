    ...
    <Shell title="保险需求详情 / Detalhe da necessidade">
      <div className="stack">
        <div className="toolbar-right">
          <Link className="button" href={`/requirements/${id}/edit`}>编辑需求 / Editar</Link>
          <Link className="button secondary" href="/requirements">返回需求列表 / Voltar</Link>
          <DeleteButton endpoint={`/requirements/${id}`} label="删除需求 / Excluir" confirmMessage="确认删除这个需求及其报价、分配记录吗？ / Confirmar exclusão desta necessidade, cotações e distribuições?" redirectTo="/requirements" />
        </div>
        <EntityDetail endpoint={`/requirements/${id}`} fields={[
