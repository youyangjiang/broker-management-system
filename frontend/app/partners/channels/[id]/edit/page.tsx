...
export default async function EditChannelPartnerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  return (
    <Shell title="编辑渠道商 / Editar parceiro de canal">
      <EntityEditForm
        endpoint={`/channel-partners/${id}`}
        redirectTo={`/partners/channels/${id}`}
        fields={[
          { name: "partner_type", label: "类型 / Tipo", type: "select", options: [{ value: "company", label: "企业 / Empresa" }, { value: "individual", label: "个人 / Pessoa física" }], required: true },
